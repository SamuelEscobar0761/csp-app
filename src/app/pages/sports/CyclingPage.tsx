...
export const CyclingPage = () => {
    ...
    useEffect(() => {
        LocateImageService.getInstance().getImages("cycling_page", "carousel")
            .then(images => {
        ...
        LocateImageService.getInstance().getImages("cycling_page", "information")
            .then(images => {
    ...
    return(
        <div>
            <ImageSlider images={imagesCarousel} text={t('cycling_page.cycling_page_title')} />
            <h2 className={`py-10 text-primary bg-white text-center text-4xl md:text-5xl lg:text-7xl font-bold`}>{t('cycling_page.information_title')}</h2>
            {imagesInfo.length === 0 ? (
